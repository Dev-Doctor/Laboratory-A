...
public enum Windows {
     ...
     */
    LOGIN(BookRecommender.PROGRAM_NAME + " - Login", "login", 250, 500),

    /**
     * Represents the registration window.
     */
    REGISTER(BookRecommender.PROGRAM_NAME + " - Register", "register", 500, 500),

    /**
     * Represents the home window.
     */
    HOME(BookRecommender.PROGRAM_NAME + " - Home", "master", 1000, 1000),

    /**
     * Represents the book overview window.
     */
    BOOK_OVERVIEW(BookRecommender.PROGRAM_NAME, "bookpage", 750, 750),

    /**
     * Represents the book review window.
     */
    BOOK_REVIEW(BookRecommender.PROGRAM_NAME + " - Write Review", "bookReview", 500, 750),

    /**
     * Represents a modal dialog for selecting a library.
     */
    LIBRARY_MODAL(BookRecommender.PROGRAM_NAME + " - Scegli una libreria", "library", 600, 200),

    /**
     * Represents the add library window.
     */
    ADD_LIBRARY(BookRecommender.PROGRAM_NAME + " Aggiungi Libreria", "addLibrary", 150, 250),

    /**
     * Represents the window for inspecting a review.
     */
    INSPECT_REVIEW(BookRecommender.PROGRAM_NAME + " - Inspect Review", "inspectReview", 600, 550),
